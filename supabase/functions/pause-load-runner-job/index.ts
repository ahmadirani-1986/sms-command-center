// pause-load-runner-job: sets pause flag.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await admin.from("load_runner_jobs")
      .update({ pause_flag: true, status: "pausing" }).eq("id", job_id);
    if (error) return json({ ok: false, error: error.message }, 500);
    await audit(admin, ctx, "load_runner_job.paused", "load_runner_job", job_id, {});
    return json({ ok: true });
  } catch (e) {
