// resume-load-runner-job: clears pause flag, sets status back to queued.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await admin.from("load_runner_jobs")
      .update({ pause_flag: false, status: "queued" }).eq("id", job_id);
    if (error) return json({ ok: false, error: error.message }, 500);
    await audit(admin, ctx, "load_runner_job.resumed", "load_runner_job", job_id, {});
    return json({ ok: true });
  } catch (e) {
